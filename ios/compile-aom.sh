...
do_lipo () {
    ...
    echo "++++++"
    echo $LIB_FILE
    echo $UNI_BUILD_ROOT/build/universal/lib/$LIB_FILE
    echo "++++++"
    LIPO_FLAGS=
    for ARCH in $FF_ALL_ARCHS
    do
        LIPO_FLAGS="$LIPO_FLAGS $UNI_BUILD_ROOT/build/aom-$ARCH/output/lib/$LIB_FILE"
    done
...
do_lipo_all () {
    ...
    echo "++++++"
    echo $UNI_BUILD_ROOT/build/universal/lib
    echo "++++++"
    ...
    cp -R $UNI_BUILD_ROOT/build/aom-arm64/output/include $UNI_BUILD_ROOT/build/universal/
}
...
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    ...
    echo "Usage:"
    echo "  compile-aom.sh armv7|arm64|i386|x86_64"
    echo "  compile-aom.sh armv7s (obselete)"
    echo "  compile-aom.sh lipo"
    echo "  compile-aom.sh all"
